...
Отчёт всегда сначала сохраняется локально. В режиме `encrypted` провайдеры
пробуются по порядку; если все недоступны, команда возвращает путь к локальному
`.pnsupport`. Незашифрованные отчёты ни одному внешнему сервису не отправляются.
