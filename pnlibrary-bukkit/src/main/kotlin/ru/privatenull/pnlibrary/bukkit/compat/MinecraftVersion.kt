...
/**
 * Известная версия Minecraft, на которой запущено Bukkit-совместимое ядро.
 *
...
enum class MinecraftVersion(val text: String, val major: Int, val minor: Int, val patch: Int) {
    ...
    companion object {
        ...
        /**
         * Определяет enum-версию запущенного сейчас Minecraft-ядра.
         * ```java
         * MinecraftVersion version = MinecraftVersion.current();
         * if (version == MinecraftVersion.UNKNOWN) {
         *     logger.warning("Неизвестная версия: " + MinecraftVersion.rawCurrent());
         * }
         * ```
         */
        @JvmStatic fun current(): MinecraftVersion = parse(rawCurrent())
        ...
        /**
         * Возвращает исходную версию Minecraft, сообщённую ядром.
         *
         * Новые ядра опрашиваются через `Server#getMinecraftVersion()`. На
         * старых серверах используется часть `Bukkit.getBukkitVersion()` до
         * первого дефиса. Метод полезен для логов и диагностики при [UNKNOWN].
         */
        @JvmStatic fun rawCurrent(): String = try {
            ...
            (method?.invoke(Bukkit.getServer()) as? String)?.takeIf { it.isNotBlank() }
                ?: Bukkit.getBukkitVersion().substringBefore('-')
        } catch (_: Throwable) { Bukkit.getBukkitVersion().substringBefore('-') }
    }
}
